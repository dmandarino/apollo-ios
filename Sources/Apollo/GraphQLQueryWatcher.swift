...
public final class GraphQLQueryWatcher<Query: GraphQLQuery>: Cancellable, ApolloStoreSubscriber {
  ...
  init(client: ApolloClient, query: Query, fetchOptions: FetchOptions, handlerQueue: DispatchQueue, resultHandler: @escaping OperationResultHandler<Query>) {
    self.client = client
    self.query = query
  ...
  func fetch(cachePolicy: CachePolicy) {
    fetching = client?._fetch(query: query, fetchOptions: fetchOptions, cachePolicy: cachePolicy, context: &context, queue: handlerQueue) { [weak self] (result, error) in
